...
- **WidgetKit extension target must use a manual `Info.plist` (`INFOPLIST_FILE`), not `GENERATE_INFOPLIST_FILE: YES`.** Xcode's generated plist omits the `NSExtension` dictionary (containing `NSExtensionPointIdentifier: com.apple.widgetkit-extension`). The Simulator installer and the App Store both require this key. Without it, `xcodebuild test` fails at install time with "extensionDictionary must be set in placeholder attributes for an app extension placeholder." The manual plist lives at `Sources/Widget/Info.plist` and must include the `NSExtension` dict; all other build-setting–injected values (`CFBundleIdentifier`, `CFBundleVersion`, etc.) use `$(BUILD_SETTING)` substitution as normal. Do not switch the widget target to `GENERATE_INFOPLIST_FILE`.
