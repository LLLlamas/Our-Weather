...
- **All on-gradient text uses `.legibleText()`.** White text on the dynamic blue/indigo/etc. background washes out without a tiny outline. The modifier in `Sources/Views/Aesthetics.swift` adds a single soft black shadow (radius 1.0, no offset, 45% opacity) — reads as a thin halo rather than a drop shadow. Apply at the `Text` / `Label` level, not at container level (otherwise the card's background gets shadowed too). `TempView` applies it internally so every temperature display is automatically outlined.
- **`WeatherBackground` darkens toward the bottom via a built-in scrim overlay.** The base condition gradient is composited with a second `.clear → .black.opacity(0.20)` top-to-bottom gradient so the lower 70% of the screen — where hourly, daily, and condition cards live — always has enough contrast for white text. The condition palettes themselves are also tuned slightly darker than the "natural" sky equivalent so daytime clear/partly-cloudy/snow gradients don't wash out white text in the upper region either. When adding a new `WeatherCondition` palette, lean toward saturated/darker stops over pastels.
...
- **WidgetKit extension plist is generated by XcodeGen, outside `Sources`.** App Store Connect (altool) rejects uploads where `NSExtension` leaks into a non-extension plist — error: "The key 'NSExtension' in the Info.plist file is not valid." Because XcodeGen auto-searches target sources for `Info.plist`, do not add a handwritten `Sources/Widget/Info.plist`. Keep the widget extension metadata in `project.yml` under the `WeatherWidget.info` block, which writes `Generated/WeatherWidget-Info.plist` during `xcodegen generate` and keeps the generated plist out of the main app target's source tree.
...
- **Night mode toggle** — moon button in the top-right toolbar (above the locations button) forces the night gradient regardless of the location's actual sunrise/sunset. Persisted via `@AppStorage("forceNightMode")` in `UserDefaults.standard`. Read by `WeatherPageView` and overrides `isDay` to `false` when on. **Does not affect the widget yet** — the widget reads from its own extension's UserDefaults, not the host app's; cross-process sync requires the App Group integration that's still on the planned list.
...
- **Thin text outline** for legibility — `.legibleText()` modifier (in `Aesthetics.swift`) wraps every white-on-gradient text with a 45%-opacity black halo, and `WeatherBackground` adds a 20% bottom-fade scrim so even daytime/snow gradients stay readable. Applied automatically by `TempView`; applied explicitly in headers, hourly time labels, daily day names, condition card labels, and all widget views.
