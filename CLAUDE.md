...
- **WidgetKit extension target must use `GENERATE_INFOPLIST_FILE: YES` with `INFOPLIST_KEY_NSExtensionPointIdentifier`.** App Store Connect (altool) rejects uploads where `NSExtension` appears in a handwritten `INFOPLIST_FILE` source plist — error: "The key 'NSExtension' in the Info.plist file is not valid." The fix is to let the build system own the plist entirely: set `GENERATE_INFOPLIST_FILE: YES` and declare the extension point via `INFOPLIST_KEY_NSExtensionPointIdentifier: com.apple.widgetkit-extension`. Xcode 26 injects the correct `NSExtension` dict from this key, satisfying both the Simulator installer and App Store Connect. `Sources/Widget/Info.plist` is still present in the repo but is excluded from the XcodeGen source list (`excludes: ["Info.plist"]`) so it is not copied into the bundle — only the generated plist is used. Do not switch back to `INFOPLIST_FILE`.
